...
const arUzpildytiIdData = () => {
  if (!CompetitionID.value) return false;
  return true;
};
...
const urlGet = "https://localhost:7134/api/Competition/GetAllCompetitions";
const urlCreate = "https://localhost:7134/api/Competition/CreateCompetition";
const urlDel = "https://localhost:7134/api/Competition/Competition/delete/";
const urlUpdate = "https://localhost:7134/api/Competition/Competitions/update/";
  ...
  function viewData() {
    fetch(urlGet, optionsGet)
      ...
      a.forEach((element) => {
        ...
        = `<tr><td> ${element.competitionID}</td>
               <td>${element.title}</td>
               <td>${element.number}</td>
               <td>${element.competitionType}</td>
               <td>${element.arenaType}</td>
               <td>${element.eId}</td>
...
function editData() {
  ...
  fetch(urlUpdate+obj.CompetitionID,{
    method: "put",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
...
function sendDataDel() {
  ...
  fetch(urlGet, optionsGet)
    .then((response) => response.json())
    .then((a) => {
      return fetch(urlDel+obj.CompetitionID, optionsDel);
    })
    .then((obj) => {
      const res = obj;
      return res;
...
userDelFormSbmBtn.addEventListener("click", (e) => {
  e.preventDefault();
  if (arUzpildytiIdData()) {
    if (confirm(`Delete? ${CompetitionID.value} entry`) == true) {
      sendDataDel();
